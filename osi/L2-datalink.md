...
> **定位**：在两个节点之间建立点对点链路，提供链路建立、认证、多协议复用功能的标准数据链路协议。是拨号上网时代和现代宽带接入（PPPoE）的核心协议。

PPP 解决三个核心问题：
- **封装**：在点对点链路上标准化封装多协议数据（IP / IPv6 / IPX 等），通过 Protocol 字段区分
- **认证**：提供 PAP / CHAP / EAP 认证机制（拨号计费、宽带接入鉴权的基础）
- **协商**：通过 LCP / NCP 动态协商链路参数和网络层地址（拨号上网时由 BRAS 分配 IP）
...
> **定位**：ISO 定义的比特导向数据链路协议，是同步串行链路上最经典的链路层封装，也是 PPP 帧格式的源头。

HDLC 解决的核心问题：
- **同步链路上的帧定界**：用 0x7E 标志位 + 比特填充实现透明传输，是早期 T1/E1、租用线路、ISDN 的标准封装
- **流量与差错控制**：通过 S 帧（RR/RNR/REJ）实现滑动窗口，提供可靠传输
- **链路管理**：通过 U 帧（SABM/UA/DISC）建立、维持、释放数据链路

现状：标准 HDLC 无协议字段、不能多协议复用，Cisco 私有版加了 Protocol 字段成为路由器串口默认封装；现代跨厂商互联一般改用 PPP。
...
> **定位**：面向连接的 L2 分组交换技术，通过虚电路（PVC/SVC）在运营商 WAN 上为多个企业站点提供互联。是 90 年代取代租用专线的企业 WAN 主流方案。

Frame Relay 解决的核心问题：
- **多站点经济互联**：用虚电路（DLCI）替代租用专线，一条接入线复用多条 PVC，显著降低 N 站点互联的线路成本
- **统计复用**：按需分配带宽，不占用的带宽可给其他虚电路使用（专线是固定独占）
- **轻量协议**：去掉了 X.25 繁重的逐跳纠错（误码率降低后不再需要），只在端到端检错，转发效率大幅提升
- **拥塞通知**：通过 FECN/BECN 字段向端点通告拥塞，由端系统自行减速

> 现状：已被 MPLS VPN（更灵活、支持流量工程）取代，仅部分遗留网络使用。
...
> **定位**：面向连接、定长信元交换技术，90 年代为承载语音/视频/数据统一业务（B-ISDN）而设计，曾长期占据运营商骨干和接入网（ADSL 早期即 ATM over DSL）。

ATM 解决的核心问题：
- **统一承载多业务**：用固定 53 字节信元 + QoS 等级（CBR/VBR/ABR/UBR），让语音、视频、数据在同一张网承载并保证时延
- **硬件高速交换**：定长信元让交换只需查 VPI/VCI 表 + 固定偏移，无需变长帧处理，硬件实现简单
- **时延可预测**：定长 + 小信元避免长帧阻塞短帧，适合实时业务

> 现状：53 字节包头开销大（~10%）、AAL5 拆装 IP 包复杂、以太网速率指数增长反超，ATM 已基本淘汰，被 MPLS + 以太网取代。
...
> **定位**：交换式以太网防环协议家族。在二层冗余拓扑中自动计算一棵无环的生成树，阻塞冗余链路以防止广播风暴，同时在主链路故障时快速切换到备份链路。
...
> **定位**：链路聚合控制协议。把多条物理链路在两端协商捆绑成一条逻辑链路（Eth-Trunk / Port-Channel），在提升带宽的同时提供链路级冗余，单条物理链路故障时流量自动切换。
...
> **定位**：跨厂商标准的二层邻居发现协议。设备周期性通告自身的身份、端口、能力等信息，供网管系统/SDN 控制器自动发现和绘制二层拓扑，是网络自动化的拓扑数据来源。
...
> **定位**：基于端口的网络接入控制（NAC）标准。在设备接入交换机/无线 AP 时强制进行身份认证，未通过认证的设备只能收发 EAPoL 帧，无法访问网络——是企业网准入控制的基础。
...
> **定位**：在 IP 网络上隧道化传输二层帧的协议，让远端用户的 PPP 会话（或任意 L2 帧）能跨越 IP 网络延伸到企业内网网关，是远程访问 VPN 和运营商 L2VPN 的隧道封装协议。

L2TP 解决的核心问题：
- **PPP 会话延伸**：把用户到本地 ISP 的 PPP 会话，通过隧道接到企业 LNS，使企业能对远程用户做统一认证/计费/寻址（如同直连内网）
- **L2VPN 伪线**：L2TPv3 能隧道化以太网/HDLC/FR/ATM 任意二层帧，在运营商 IP 网上模拟一条点对点 L2 链路
- **解耦"封装"与"加密"**：L2TP 本身只负责隧道封装不加密，需叠加 IPsec 才有保密性（L2TP/IPsec 经典组合）
